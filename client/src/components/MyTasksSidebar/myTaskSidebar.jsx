  ...
  const [showMyTasks, setShowMyTasks] = useState(false)
   ...
   return(
    <div className="mt-6 px-3">
     <div onClick={toggleMyTasks} className="flex items-center justify-between px-3 py-2 rounded-lg cursor-pointer 
     hover:bg-gray-100 dark:hover:bg-zinc-800">
      
     <div className="flex items-center gap-2">
      <CheckSquareIcon size={25} className=" text-gray-500 dark:text-zinc-400"/>
      <h3 className="text-md font-medium text-gray-700 dark:text-zinc-300">
        My Tasks
      </h3>

      <span className="bg-gray-200 dark:bg-zinc-700 text-gray-700 dark:text-zinc-300 text-sm px-2 py-0.5 rounded">
        {myTasks.length}
      </span>
     </div>
     ...
     {showMyTasks && (
      <div className="mt-2 pl-2">
       {myTasks.length === 0 ? (
        <div className="px-3 py-2 text-md text-gray-500 dark:text-zinc-500 text-center">
          Não há tarefas   
        </div>
       ) : (
       <div className="space-y-1">
        {myTasks.map((task) => (
         ...
         className="w-full rounded-lg transition-all duration-200 text-gray-700 dark:text-zinc-300 hover:bg-gray-100 dark:hover:bg-zinc-800
         ...
         <div className="flex items-center gap-2 px-3 py-2 w-full min-w-0">
          <div className={`w-2 h-2 rounded-full ${getTaskStatusColor(task.status)} shrink-0`}>
           <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-gray-500 dark:text-zinc-500 lowercase">
